...
class aplicativo():
    def __init__(self):
        self.window = Tk()
        self.window.title("FLProgram - SUPERMERCADO HELY")
        self.window.geometry("800x600")
        self.window.state("zoomed")
        self.background = "#ff1510"
        self.window["bg"] = self.background
        self.atual_window = "init"
        self.heigh = 2
        self.icone = "c:\FLPrograms\Read\Config\Icone.ico"
        self.last_event = ""
        self.scan_window = self.window.bind("<FocusIn>", self.scan)
        self.hotkey()
        self.diretorios()
        self.DB()
        #self.connect()
        ...
        self.btnfechamento = Button(self.window,text="Fechamento de caixa", font=self.fontebotao , width=18,height=self.heigh, bg="yellow",command=self.fechamento)
        self.btnfechamento.pack(anchor=N, side=LEFT)
        self.btnfechamento.focus()
        self.btncont = Button(self.window,text="Contar dinheiro",bg="yellow", width=19, height=self.heigh, font=self.fontebotao , command=self.contarDinheiro)
        self.btncont.pack(anchor=N, side=LEFT)
        self.btnatacado = Button(self.window,text="Atacado", bg="yellow", width=19, height=self.heigh,font=self.fontebotao, command=self.atacado)
        self.btnatacado.pack(anchor=N, side=LEFT)
        self.btnadmin = Button(self.window,text="Administrativo", width=19, height=2,font=self.fontebotao, background="yellow" , command=self.confirmar)
        self.btnadmin.pack(anchor=N, side=LEFT)
        self.btnfechar = Button(self.window,text="Fechar",bg="yellow", width=19, height=2 , font=self.fontebotao ,command=self.window.destroy)
        self.btnfechar.pack(anchor=N,side=LEFT)
        self.lbll = Label(self.window,text='', height=2, font="arial 18 bold",bg="yellow")
        self.lbll.pack(anchor=N, fill=X)
    ...
    def hotkey(self):
        ...
        def f_enter():
            ...
            if self.last_event[:5] == "entry":
                keyboard.press("tab")
            elif self.last_event[:6] == "button":
        ...
        self.enter = keyboard.add_hotkey("enter", f_enter)
        self.s_enter = keyboard.add_hotkey("shift+enter", s_enter)
        #if window == "init":
        #self.s_enter = keyboard.add_hotkey("shift+enter", lambda:keyboard.press("shift+tab"))
        #self.enter = keyboard.add_hotkey("enter",f_enter)
        #elif window == "fechamento":
        #    try:
        #        self.s_enter.destroy()
        #        self.enter.destroy()
        #    except:
    ...
    def atacado(self):
        self.window.destroy()
        self.window1 = Tk()
        btnvoltar = Button(self.window1,text="Voltar", height=1, font=self.fontebotao, width=4, command=self.voltar)
        btnvoltar.pack(anchor=NW)
        text = """
---@@@@@@
-@--------@
@---@--@---@
...
--@@@@@@
 """
        txt = Label(self.window1, text= text, font="arial 12 bold")
        txt.pack()
        self.window1.mainloop()
    
    def imprimir(self, arquivo, local):
        #"c:\FLPrograms\Read\Config\Texts"
        win32api.ShellExecute(0, "print" , arquivo ,  None , local , 0  )
    ...
    def connect(self):
        with open(r"c:\FLPrograms\Read\config\Texts\config.config","r") as config:
        ...
        def atualizar(local,terminal):
            bank1 = sqlite3.connect(r"C:\FLPrograms\DBSupermercado Hely.sql") 
            cursor1 = bank1.cursor()
            bank2 = sqlite3.connect(local)
            cursor2 = bank2.cursor()
            cursor1.execute("SELECT dia, caixa, dinheiro, cartao, aprazo, pix FROM fechamento")
            bnk1 = cursor1.fetchall()
            
            cursor2.execute("SELECT dia, caixa, dinheiro, cartao, aprazo, pix FROM fechamento")
            bnk2 = cursor2.fetchall()
            jnlt = Tk()
            jnlt.title(f"Atualizando Terminal {terminal}")
            jnlt.geometry("300x20")
            jnlt.resizable(0,0)
            status = DoubleVar()
            status.set(0)
            b_status = ttk.Progressbar(jnlt, variable=status, maximum=len(bnk2))
            b_status.pack(fill=X)
            a=0
            for i in bnk2:
                a+=1
                status.set(a)
                jnlt.update()
                if i in bnk1: continue
                else: 
                    print(i)
                    cursor1.execute(f"INSERT INTO fechamento(dia, caixa, dinheiro, cartao, aprazo, pix)values('{i[0]}','{i[1]}',{i[2] if i[2] != None else 0},{i[3] if i[3] != None else 0},{i[4] if i[4] != None else 0},{i[5] if i[5] != None else 0})")
                    self.bankfechamento.commit()
            cursor1.execute("SELECT dia, gastosSemRetorno, valorSR, gastosComRetorno, valorCR, funcionario FROM gastos")
            bnk1 = cursor1.fetchall()
            cursor2.execute("SELECT dia, gastosSemRetorno, valorSR, gastosComRetorno, valorCR, funcionario FROM gastos")
            bnk2 = cursor2.fetchall()
            a = 0
            for i in bnk2:
                a += 1
                ...
                jnlt.update()
                if i in bnk1:
                    if bnk2.index(i)+1 == len(bnk2):
                        jnlt.destroy()
                    continue
                else:
                    cursor1.execute(f"INSERT INTO gastos(dia, gastosSemRetorno, valorSR, gastosComRetorno, valorCR, funcionario)values('{i[0]}','{i[1]}',{i[2] if i[2] != None else 0},{i[3] if i[3] != None else 0},{i[4] if i[4] != None else 0},{i[5] if i[5] != None else 0})")
                    bank1.commit()
                    if bnk2.index(i)+1 == len(bnk2):
                        jnlt.destroy()
            jnlt.mainloop() 
        ...
        for i in list_conect :
            ...
            #try:
            if online(i):
                atualizar(i,terminal)
            else: print("teminal " + str(terminal) + " offline")
            #except: 
                #FAIL IN CHECK DB
            #    confirm = messagebox.showerror(title="OFFLINE",message=f"ERRO COM CARREGAMENTO COD:FL-ODB-0{terminal}")
    ...
    def DB(self):
        ...
        def fail():
            ...
            if erro:
                astcnica = messagebox.askyesno(title="ERRO", message="Confirmar ir à area de restauração")
                if astcnica:
                    ast = Tk()
        ...
        try:
            def check_db():
                try :
                    ...
                    return False
            if check_db() :
                self.bankfechamento = sqlite3.connect(r"C:\FLPrograms\DBSupermercado Hely.sql")
                self.bankgastos = self.bankfechamento
    ...
    def fechamento(self):
        ...
        self.window1["bg"] = self.background
        self.scan_window = self.window1.bind("<FocusIn>", self.scan)
        self.atual_window = "fechamento"
        ...
        if self.valortarde > 0 :
            self.txtarde1 = Label(self.window1,text=f"{format(self.valortarde,'.2f').replace('.',',')}", bd=1, anchor=E ,width=9, font=self.fonteLabel, bg="white")
...
TOTAL                       {format(self.dinheirocsuprimento,'.2f')}
SUPRIMENTO                - {self.suprimento}
______________________________________
TOTAL                       {format(self.dinheirototal,'.2f')}
CARTÃO                      {self.cartao}
A PRAZO                     {self.aprazo}
PIX / TICKET                {self.pix}
______________________________________
...
"""))
    ...
    def imprimir(self):
        try:
            ...
            win32api.ShellExecute(0, "print" , "Fechamento impresso.txt" ,  None , caminho , 0  )
            #os.startfile("c:\FLPrograms\Read\Config\Texts\Fechamento impresso.txt","print")
        except:
    ...
    def guardar(self):
        ...
        if confirm:
            ...
            dia = self.dia
            caixa = self.cx.lower()
            dinheiro = format(self.dinheirototal,".2f")
            cartao = format(self.cartao,".2f")
            aprazo = format(self.aprazo,".2f")
            pix = format(self.pix,".2f")
    ...
    def contarDinheiro(self):
        ...
        self.window2["bg"] = self.background
    ...
    def verificar(self):
            ...
            self.cursorfechamento.execute("SELECT supervisor FROM keys")
            ...
            for row in lista:
                ...
                if user == "felipe " and passwd =="FLProgram":
                    ...
                    self.cursorfechamento.execute("SELECT senha FROM keys")
    ...
    def Programador(self):
        ...
        self.jnl3["bg"] = self.background
        btnprint = Button(self.jnl3,text="Impressora padrão", font=self.fontebotao , bg="yellow",height=self.heigh, width=15, command=self.print_padrao)
        btnprint.pack(anchor=N, side=LEFT)
        btnsupervisor = Button(self.jnl3, text="Novo Supervisor", font = self.fontebotao ,bg="yellow",height=self.heigh, width=15, command=self.new_super)
        btnsupervisor.pack(anchor=N, side=LEFT)
        btndb = Button(self.jnl3, text="Terminais" , font=self.fontebotao , bg="yellow",height=self.heigh, width=15 , command=self.cadastrarservidor)
        btndb.pack(anchor=N, side=LEFT)
        btnvoltar = Button(self.jnl3, text="Voltar", bg="yellow",height=self.heigh, width=15, font=self.fontebotao, command=self.voltar)
        btnvoltar.pack(anchor=N, side=LEFT)
        ...
        def cadastrar():
            ...
            for nomes in supervisores:
                if nomes[0] == nome:
                    ...
                    if repetido:
                        self.cursorfechamento.execute(f"UPDATE keys SET senha = '{senha}' WHERE supervisor = '{nome}'")
                        self.bankfechamento.commit()
                        jnl6.destroy()
                        break
                    else:
                    ...
                    if confirma:
                        ...
                        self.cursorfechamento.execute(f"INSERT INTO keys (supervisor , senha) VALUES ('{nome}','{senha}')")
                        self.bankfechamento.commit()
                        jnl6.destroy() 
                        break
                else:
        ...
        def apagar():
            ...
            for name in supervir:
                if name[0] == nome:
                    ...
                    if askdelete:
                        self.cursorfechamento.execute(f"DELETE FROM keys WHERE supervisor = '{nome}' ")
                        self.bankfechamento.commit()
                        jnl6.destroy()
                        break
                elif control == control1 :
    ...
    def cadastrarservidor(self):
        ...
        alterar = messagebox.askyesno(title="CADASTRAR TERMINAIS", message="Selecionar Local dos demais terminais")
        if alterar:
            cd = Tk()
            cd.geometry("300x70")
            ...
            inf.pack(side=TOP)
            eqt = Entry(cd,font="arial 12 bold")
            eqt.pack(side=LEFT)
            list_terminais = []
            def cadastrar_terminal(qntd_terminais):
                cd.destroy()
                for i in range(0,qntd_terminais):
                    messagebox.showinfo(title=f"Terminal {i+1}", message=f"Informar caminho do terminal {i+1}")
                    termdb = filedialog.askopenfilenames()[0]
                    if termdb != None:
                        list_terminais.append(str(termdb))
                        if i+1 == qntd_terminais: 
                            with open(r"c:\FLPrograms\Read\Config\Texts\Config.config" , "w") as dbl:
                                for i in list_terminais:
                                    dbl.write(str(i)+"\n")
                            print(list_terminais)
                    else: break
            bt = Button(cd,text="Buscar",command=lambda:cadastrar_terminal(int(eqt.get())))
            bt.pack(side=LEFT)
            cd.mainloop()
    ...
    def administrador(self):
        ...
        jnl2.state("zoomed")
        jnl2["bg"] = self.background
        #titulo = Label(jnl2,text="ADIMINISTRATIVO", width=20, bg=self.background, font=self.fontetitulo)
        #titulo.pack(anchor=CENTER, fill=X, pady=20)
        
        def gastos():
            jnl2.destroy()
            self.gerirgastos()
        
        btngast = Button(jnl2,text="Gerenciar Gastos", bg="#ff0", width=20 ,height=self.heigh, font=self.fontebotao, command=gastos)
        btngast.pack(anchor=N,side=LEFT)
        btnrelatorio = Button(jnl2,text="Consultar Relatorios", bg="#ff0",width=20, height=self.heigh, font=self.fontebotao, command=self.relatorio)
        btnrelatorio.pack(anchor=N, side=LEFT)
        jnl2.iconbitmap(self.icone)
        lbl_barra = Label(jnl2,text="", bg="#ff0", height=5, font="arial 7", bd=1, relief=SOLID)
        lbl_barra.pack(fill=X, anchor=N)
        jnl2.mainloop()
    ...
    def relatorio(self):
        ...
        for dt in dias:
            idce = dias.index(dt)
            tventrada.insert("", "end", values=(dt, format(moneys[idce],'.2f'), format(cards[idce],'.2f'), format(pixs[idce],'.2f')))
    ...
    def imprimir_relatorio(self,arquivo):
        ...
        def printer():
            ...
            caminho = f"c:\FLPrograms\Relatorios\{arquivo} "
